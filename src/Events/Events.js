  ...
  constructor(props) {
    ...
    this.panTopButtonResponder = PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        const _topIndex =
          Math.floor(
            (4 *
              (((this.state.bottomTimeIndex - this.props.minHour * 4) / 4) *
                this.offset -
                this.height.current +
                gestureState.dy)) /
            this.offset,
        ...
        if (
          _topIndex >= this.state.bottomTimeIndex ||
    ...
    this.panBottomButtonResponder = PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        ...
        if (
          _bottomIndex <= this.state.topTimeIndex ||
    ...
    this.props.onTimeIntervalSelected &&
      this.props.onTimeIntervalSelected(
        moment(initialDate)
          ...
          .startOf('day')
          .add(Math.floor(this.state.topTimeIndex / 4), 'hours')
          .add((this.state.topTimeIndex % 4) * 15, 'minutes')
          .toDate(),
        moment(initialDate)
          .add(this.state.dayIndex, 'day')
          .startOf('day')
          .add(Math.floor(this.state.bottomTimeIndex / 4), 'hours')
          .add((this.state.bottomTimeIndex % 4) * 15, 'minutes')
          .toDate(),
      );
  };
